...
psql -d fdd2db -c 'select user_id, count(repetitions) as number_of_inter_genre_repetitions_in_first_decile from (select user_id, genre, average, decile_interval, count(user_id) over(partition by user_id) as repetitions from possible_bots) as sbq group by user_id order by number_of_inter_genre_repetitions_in_first_decile desc;'
