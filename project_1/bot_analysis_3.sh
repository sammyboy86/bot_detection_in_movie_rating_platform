...
psql -d fdd2db -c 'select genre, cast(count(case when repetitions > 1 then 1 else null end) as float) / cast(count(user_id) as float) as percentage_of_inter_genre_ids_in_first_decile from (select user_id, genre, average, decile_interval, count(user_id) over(partition by user_id) as repetitions from possible_bots) as sbq group by genre'
